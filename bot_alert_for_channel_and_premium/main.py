...
driver = webdriver.Firefox(
    executable_path="C:\\Users\\Владислав\\Bet_bot\\geckodriver.exe",
    options=options
)
...
def get_bet(coef, lose_money):
    for bet in range(1, 100000000):
        if bet * coef >= bet + lose_money + 100:
            return bet
                    ...
                    if coef.replace(".", "").isdigit():
                        ...
                        if (coef > 1.5) and (coef < 2.1):
                            ...
                            while True:
                                ...
                                if _dict['Date'] == "Сейчас":
                                    time.sleep(7200)

                                elif _dict['Date'] == "Через":
                                    _time = 7200 + (_dict['Time'] * 60)
                                    time.sleep(_time)

                                elif _dict['Date'] == "Сегодня":
                                    now = datetime.now()
                                    current_time = int(now.strftime("%H"))
                                    _time = 7200 + (_dict['Time'] - current_time) * 60 * 60
                                    time.sleep(_time)

                                elif _dict['Date'] == "Завтра":
                                    now = datetime.now()
                                    current_time = int(now.strftime("%H"))
                                    _time = 7200 + ((24 - current_time) + _dict['Time']) * 60 * 60
                                    time.sleep(_time)
